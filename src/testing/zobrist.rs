...
use crate::{util::zobrist::ZobristHashMap, rules::board::Board};
...
impl ZobristCollisionTestContext {
    ...
    pub fn process(&mut self, board: Board) {
        ...
        match self.all_boards.get_mut(&board.id) {
            ...
            None => {
                self.all_boards.insert(board.id, FxHashSet::from_iter([fen].into_iter()));
                self.memory_used += mem::size_of::<u64>() as u64;
                self.memory_used += mem_size as u64;
            },
        }
...
impl ZobristCollisionTester {
    pub fn do_test(board: Board, depth: u8) -> ZobristCollisionTestResult {
        ...
        ctx.init();
        Self::collision_test(board, depth, &mut ctx);
        return ctx.complete();
    }

    pub fn collision_test(board: Board, depth: u8, ctx: &mut ZobristCollisionTestContext) {
        ctx.process(board);
        if depth <= 0 { return; }
        for mov in board.get_legal_moves() {
            let mut new_board = board;
            new_board.make_move(&mov);
            Self::collision_test(new_board, depth - 1, ctx)
        }
    }
}